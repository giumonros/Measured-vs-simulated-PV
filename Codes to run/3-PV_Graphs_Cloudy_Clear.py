...
clear_sky_df = pd.read_excel(file_path, sheet_name='Clear sky')
cloudy_sky_df = pd.read_excel(file_path, sheet_name='Cloudy sky')
...
def plot_data(df1, df2):
    ...
    axs[0].set_title('Forlì 2014 - Clear Sky Day', fontsize=20)
    ...
    for i, column in enumerate(column_names[1:]):  # Iterate only over columns of interest
        axs[1].plot(time_series2, df2[column], label=legend_names[i],
                    color=colors[i], linestyle=linestyles[i], linewidth=line_widths[i])
    axs[1].set_title('Forlì 2014 - Cloudy Sky Day', fontsize=20)
    ...
    plt.savefig(f'{location_name}_hourly_graph.png', bbox_inches='tight')
    plt.show()
