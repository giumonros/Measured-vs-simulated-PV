...
output_dir_timeseries = os.path.join(
    output_dir, location_name, "Time series analysis results"
)
...
file_path = os.path.join(
    "Simulated and measured PV data", f"{location_name}_meas_sim.csv"
)
data_sim_meas = pd.read_csv(file_path, header=None).iloc[
    :, 1:
]  # Skip the first column and load without headers
...
def convert_comma_to_dot(df):
    for column in df.columns:
        ...
        ):  # Checks if the column is of type object (i.e., strings)
            df[column] = df[column].str.replace(",", ".")  # Replaces commas with dots
            try:
                df[column] = df[column].astype(
                    float
                )  # Attempts to convert the column to float
            except ValueError:
...
data_sim_meas.columns = pd.MultiIndex.from_tuples(
    tuple(zip(new_headers.iloc[0], new_headers.iloc[1]))
)
...
data_sim_meas = data_sim_meas.apply(
...
data_sim_meas_filtered_cloudy = data_sim_meas[
    [col for col in data_sim_meas.columns if year_selected_cloudy in col]
]
data_sim_meas_filtered_cloudy = (
    data_sim_meas_filtered_cloudy.reset_index()
)  # Adds a new column 'index' with row numbers starting from 0
data_sim_meas_filtered_cloudy.rename(
    columns={"index": "Hour of the year"}, inplace=True
)  # Rename for matching
data_sim_meas_filtered_cloudy["Hour of the year"] = range(
    1, len(data_sim_meas_filtered_cloudy) + 1
)  # Makes hour of the year start from 1

# Merge with the cloudy sky DataFrame
cloudy_sky_df = cloudy_sky_df.merge(
    data_sim_meas_filtered_cloudy, on="Hour of the year", how="left"
)
...
data_sim_meas_filtered_clear = data_sim_meas[
    [col for col in data_sim_meas.columns if year_selected_clear in col]
]
data_sim_meas_filtered_clear = (
    data_sim_meas_filtered_clear.reset_index()
)  # Adds a new column 'index' with row numbers starting from 0
data_sim_meas_filtered_clear.rename(
    columns={"index": "Hour of the year"}, inplace=True
)  # Rename for matching
data_sim_meas_filtered_clear["Hour of the year"] = range(
    1, len(data_sim_meas_filtered_clear) + 1
)  # Makes hour of the year start from 1

# Merge with the clear sky DataFrame
clear_sky_df = clear_sky_df.merge(
    data_sim_meas_filtered_clear, on="Hour of the year", how="left"
)
# Remove the first two columns
...
# Define a dictionary with color mappings for CF
color_mapping_CF = {
    "PV-MEAS": "red",
    "MERRA2": "blue",
    "SARAH3": "sienna",
...
# Define a dictionary with line style mappings for CF
...
# Default line style if no match is found
default_linestyle_CF = "-."

# Define a dictionary with color mappings for high resolution graph
color_mapping_high_res = {
...
# Default color if no match is found
default_color_high_res = "darkgreen"

# Define a dictionary with line style mappings for high resolution
linestyle_mapping_high_res = {
...
# Default line style if no match is found
default_linestyle_high_res = "-."

for name in legend_names:
    # Determine the line style based on the keywords
    linestyle_CF = next((linestyle for keyword, linestyle in linestyle_mapping_CF.items() if keyword in name), default_linestyle_CF)
    linestyles_CF.append(linestyle_CF)

    # Determine the color based on the keywords
    color_CF = next((color for keyword, color in color_mapping_CF.items() if keyword in name), default_color_CF)
    colors_CF.append(color_CF)

for name in legend_names_high_res:
    # Find the line style by checking if any dictionary key is a substring of the name
    linestyle_high_res = next((linestyle for keyword, linestyle in linestyle_mapping_high_res.items() if keyword in name), default_linestyle_high_res)
    linestyles_high_res.append(linestyle_high_res)

    color_high_res = next((color for keyword, color in color_mapping_high_res.items() if keyword in name), default_color_high_res)
...
for location in locations:
    ...
    loc_data = data_sim_meas[
        [col for col in data_sim_meas.columns if col.startswith(location)]
    ]
    ...
    loc_data_sorted = loc_data.apply(
        lambda x: x.sort_values(ascending=False).reset_index(drop=True)
    )
    ...
    plt.savefig(
        os.path.join(output_dir_timeseries, f"{location}_Capacity_Factors.png"),
        bbox_inches="tight",
    )
    print(
        f"Capacity factors figure successfully generated in the '{output_dir}' folder for {location}"
    )
    plt.close()
    ...
    print(
        f"Scatter plot figure successfully generated in the '{output_dir}' folder for {location}"
    )
    plt.close()
    ...
    for sim_col in valid_columns:
        if sim_col != meas_column and any(
            tool in sim_col for tool in plot_palette.keys()
        ):
            ...
            if not simulated_data_err_an.empty:
                mean_diff = (
                    simulated_data_err_an.mean() - measured_data_err_an.mean()
                ) * 100
                mae = (
                    mean_absolute_error(measured_data_err_an, simulated_data_err_an)
                    * 100
                )
                rmse = (
                    np.sqrt(
                        mean_squared_error(measured_data_err_an, simulated_data_err_an)
                    )
                    * 100
                )
                mean_diff_results.append(
                    {
                        "Location": location,
                        "Tool": sim_col.split()[1],
                        "Mean Difference (%)": mean_diff,
                    }
                )
                mae_results.append(
                    {"Location": location, "Tool": sim_col.split()[1], "MAE (%)": mae}
                )
                rmse_results.append(
                    {"Location": location, "Tool": sim_col.split()[1], "RMSE (%)": rmse}
                )
    ...
    for p in ax.patches:  # Loop through all bars in the subplot
        ...
        if abs(height) > 1e-3:  # Ignore near-zero bars to prevent "0.0" labels
            if height >= 0:
                ...
                ax.annotate(
                    format(height, ".1f"),  # Format the label to one decimal place
                    (
...
handles, labels = axes[
    2
].get_legend_handles_labels()  # Use handles from the last subplot
...
combined_plot_path = os.path.join(
    output_dir_timeseries, f"{location_name}_Errors_Analysis.png"
)
plt.savefig(combined_plot_path, bbox_inches="tight")
print(
    f"Combined error analysis figure successfully generated in the '{output_dir}' folder"
)
plt.close()
...
def plot_data(df1, df2):
    ...
    for i, column in enumerate(
        column_names_df2
    ):  # Iterate only over all columns in df2
        # Check if the legend matches a valid entry
        if column.split()[1] in legend_names_high_res:  # Match against the legend
            idx = legend_names_high_res.index(
                column.split()[1]
    ...
    plt.savefig(
        os.path.join(output_dir_timeseries, f"{location_name}_sec_vs_hourly_graph.png"),
        bbox_inches="tight",
    )
    plt.close()
    print(
        f"High resolution PV data figure successfully generated in the '{output_dir}' folder for {location}"
    )
