...
file_path = os.path.join("Time series analysis graphs", "Input files", f"{location_name}_meas_sim.csv")
...
output_dir = 'Output graphs saved'
...
for location in locations:
    ...
    # Further filter to exclude any simulation columns with all zero values
    ...
    for idx, tool in enumerate(legend_names):
        ...
        if tool_column in loc_data_sorted.columns:
            plt.plot(loc_data_sorted[tool_column], label=tool, color=colors[idx],
                     linestyle=linestyles[idx], linewidth=line_widths[idx])

    plt.title(f'{location} - Capacity Factors', fontsize=20)
